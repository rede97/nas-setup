...
def toml_gen_elem(inst: Any) -> str:
    if isinstance(inst, str):
        ...
        content = ", ".join(map(lambda k: f"{k} = {toml_gen_elem(inst[k])}", inst))
        return f"{{ {content} }}"
...
def toml_gen_elem_table(
    w: StringIO, inst: dict | list[dict], name: Optional[str] = None
):
...
def trim_general_config(
    content: str, comment: set[str] = {"#"}
) -> dict[str | None, str]:
    config_dict = {}
    tag = None
...
def trim_general_config_file(
    p: Path, comment: set[str] = {"#"}
) -> dict[str | None, str]:
    with open(p, "rt") as f:
